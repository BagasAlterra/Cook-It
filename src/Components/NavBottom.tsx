...
const NavBottom = () => {
  ...
  const endpoint = `https://cookit.altapro.online/users`;
  ...
  const getcart = () => {
    axios
      .get("https://cookit.altapro.online/users/carts", {
        headers: {
          Authorization: `Bearer ${cookies.user.token}`,
        },
      })
    ...
    >
      ...
      >
        <button
          ...
          className="font-bold hidden sm:flex sm:text-sm lg:font-semibold lg:text-4xl"
        >
          Cookit
        </button>
        ...
        <div className={`hidden dropdown dropdown-bottom sm:block `}>
          <label
            ...
            } hover:cursor-pointer  sm:flex items-center gap-2 hover:text-secondary`}
          >
            <div
          ...
          <ul
            tabIndex={0}
            className="dropdown-content md:text-sm menu p-2 text-black shadow-lg bg-base-100 rounded-box w-52"
          >
            <li>
              <Link to={`/profile/${cookies.user.id}`}>Profile</Link>
            </li>
            <li>
            ...
            <li
              className={`${
                cookies.user.role === "Admin" ? "block" : "hidden"
              }`}
            >
              <Link to="/verifyusers">verify users</Link>
            </li>
          </ul>
        </div>
        ...
        >
          <div className="indicator">
            <span className="indicator-item badge text-white badge-secondary">
              {cartLength}
            </span>
            <MdShoppingCart />
          </div>
      ...
      {modalOpen ? (
        <>
          <div
            onClick={() => setModalOpen(!modalOpen)}
            className="w-full h-full fixed inset-0 z-40 bg-black opacity-50"
          ></div>
          <div
            className="
                    ...
                    "
          >
            <p className="hidden justify-self-center sm:flex text-xl text-center font-semibold col-span-2 my-10">
              What type of Post would you like to make?
            </p>
            <Link
              to={"/newcooking"}
              onClick={() => setModalOpen(!modalOpen)}
              className="btn flex gap-2 btn-primary rounded-full"
            >
              <GiKnifeFork />
              New Cooking
            </Link>
            ...
            <button
              onClick={() => setModalOpen(!modalOpen)}
              className="btn btn-outline sm:btn-sm sm:btn-circle btn-primary rounded-full sm:absolute sm:right-4 sm:top-4"
            >
