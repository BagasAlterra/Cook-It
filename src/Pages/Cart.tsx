...
const Cart = () => {
  ...
  const handleIncrement = (price: any, id: number, quantity: number) => {
    axios
      .put(
        `https://cookit.altapro.online/users/carts/${id}`,
        {
          quantity: quantity + 1,
        },
        {
  ...
  const deleteCartItem = (id: number) => {
    axios
      .delete(`https://cookit.altapro.online/users/carts/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
            ...
            <>
                    ...
                    <>
                      <div key={key}>
                        <ItemCart
                          getIdProduct={(e) => {
                            !e.target.checked
                              ? removeData(parseInt(e.target.value))
                              : setDataCartChecked(
                                  dataCartChecked.concat(
                                    carts[
                                      idCart.indexOf(parseInt(e.target.value))
                                    ]
                                  )
                                );
                          }}
                          ...
                          increment={(price, id, quantity) =>
                            handleIncrement(price, id, quantity)
                          }
                          decrement={(price, id, quantity) =>
                            handleDecrement(price, id, quantity)
                          }
              ...
              <div className="w-full h-16 px-5 lg:px-10 grid grid-cols-3 place-content-center items-center fixed bottom-0 bg-gray-100">
                ...
                {dataCartChecked.length != 0 ? (
                  <button
                    onClick={goToPayment}
                    className="w-full md:w-2/3 lg:w-1/3 py-2 place-self-end text-center text-white font-semibold rounded-md bg-primary"
                  >
                    Checkout
                  </button>
                ) : (
                  <button className="w-full md:w-2/3 lg:w-1/3 py-2 place-self-end text-white font-semibold rounded-md bg-gray-400">
                    Checkout
                  </button>
                )}
              </div>
