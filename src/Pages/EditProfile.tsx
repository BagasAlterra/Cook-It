...
const EditProfile = () => {
  ...
  const getData = () => {
    axios
      .get(`https://cookit.altapro.online/users`, {
        headers: {
  ...
  const updatePassword = () => {
    axios.put(
      `https://cookit.altapro.online/users/password`,
      {
        old_password: oldPw,
        new_password: newPw,
  ...
  const updateProfile = async () => {
    ...
    if (formData != null) {
      axios
        .put("https://cookit.altapro.online/users", formData, {
          headers: {
            Authorization: `Bearer ${cookies.user.token}`,
  ...
  const upgradeAccount = () => {
    ...
    fetch(`https://cookit.altapro.online/users/upgrade`, headers)
        ...
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-2 bg-gray-100 px-5 mx-auto">
          {data?.map((item: any, index: number) => {
            return (
              <div
                key={index}
                className="p-3 lg:h-screen  bg-white rounded-lg my-4"
              >
                <input
                ...
                <div className="grid grid-cols-2 place-content-end items-center mb-10">
                  <label
                    ...
                    className="w-24 h-24 rounded-full cursor-pointer relative overflow-hidden flex justify-center items-center hover:brightness-50"
                  >
                    <BsFileImageFill
                      className={`${
                        isHover ? "block" : "hidden"
                      } text-4xl text-white absolute z-30`}
                    />
                    <img
                      className="rounded-full w-24 h-24"
                      src={imageUrl ? imageUrl : item.profile_picture}
                      alt=""
                    />
                  </label>
                  <button
                    onClick={updateProfile}
                    className="w-24 lg:w-32 py-1 rounded-md place-self-center bg-secondary text-white"
                  >
                    Save
                  </button>
                </div>
                <form action="" className="flex flex-col">
                  ...
                  <input
                    ...
                    type="text"
                    className="border-r border-l border-t border-b border-1 border-primary outline-none rounded-md px-2 py-1"
                  />
                ...
                <div className="text-start pt-3 text-xs">
                  ...
                  <div className="px-2">
                    <p className={`flex ${length ? "text-green-600" : ""}`}>
                      {length ? <BiCheck /> : ""} At least 8 characters with no
                      space
                    </p>
                    <p className={`flex ${upperCase ? "text-green-600" : ""}`}>
                      {upperCase ? <BiCheck /> : ""} At least 1 upper case
                      letter
                    </p>
                    <p className={`flex ${oneNumber ? "text-green-600" : ""}`}>
                      {oneNumber ? <BiCheck /> : ""} At least 1 number
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="container px-5 py-10 mx-auto">
                <div className="pt-3">
                  <h2 className="w-full text-center leading-border-text -mb-3 pr-2 pl-2">
                    <span className="bg-white text-primary text-sm font-semibold">
                  ...
                  <div className="flex flex-wrap justify-center gap-5 px-5 pb-5 pt-4 border-b border-t border-r border-l border-primary rounded-md">
                    <p>
                      Verified users gain a purple checkmark and the ability to
                      sell their recipes
                    </p>
                    <button
                      onClick={() => upgradeAccount()}
                      className="w-24 py-1 bg-secondary font-semibold rounded-md text-white"
                    >
                      Request
                    </button>
                  </div>
