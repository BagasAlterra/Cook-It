...
const Follow = () => {
  ...
  const getFollowers = async () => {
    setLoading(true);
    await axios
      .get("https://cookit.altapro.online/users/follower", {
        headers: {
  ...
  const handleFollback = async (id: number) => {
    ...
    console.log(id);
    fetch(`https://cookit.altapro.online/users/follow/${id}`, headers)
  ...
  const handleUnfollow = async (id: number) => {
    await axios
      .delete(`https://cookit.altapro.online/users/unfollow/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  const getFollowing = async () => {
    setLoading(true);
    await axios
      .get("https://cookit.altapro.online/users/following", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
    ...
    <div className="flex justify-center ">
      <div className="w-full h-full flex flex-col items-center">
        ...
        <nav className="w-full h-12 bg-primary sticky top-0 grid grid-cols-2 justify-center items-center text-center">
          <p
            onClick={() => setIsNavigate("Followers")}
            className={`text-white place-self-center cursor-pointer w-fit py-2  ${
              isNavigate == "Followers"
                ? "border-b-[3px] border-secondary font-semibold"
                : ""
            } `}
          >
            Followers
          ...
          <p
            onClick={() => setIsNavigate("Following")}
            className={`text-white place-self-center cursor-pointer w-fit py-2  ${
              isNavigate == "Following"
                ? "border-b-[3px] border-secondary font-semibold"
                : ""
            } `}
          >
