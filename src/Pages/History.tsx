...
const History = () => {
  ...
  const getData = async () => {
    ...
    await axios
      .get(
        `https://cookit.altapro.online/users/transactions?status=${router}`,
        {
          headers: {
  ...
  async function fetchData() {
    const response = await fetch(
      "https://cookit.altapro.online/users/transactions?status=Shipped",
      {
        headers: {
          Authorization: `Bearer ${cookies.user.token}`,
        },
  ...
  useEffect(() => {
    if (idTransaction.length != 0) {
      ...
      if (count === MAX_TIME) {
        ...
        fetch(
          `https://cookit.altapro.online/users/transactions/${idTransaction}/status`,
          headers
        )
  ...
  const orderReceived = (id: number) => {
    ...
    fetch(
      `https://cookit.altapro.online/users/transactions/${id}/status`,
      headers
    )
      .then((response) => response.json())
      .then((data) => {
  ...
  return (
    <div className={`bg-gray-100 ${data.length >= 1 ? "" : "h-screen"} `}>
      ...
      <div className="w-full h-16 text-gray-100 px-3 text-center flex sticky top-16 z-50 items-center gap-10 bg-primary realtive overflow-hidden">
        <Swiper spaceBetween={50} slidesPerView={screen.width > 767 ? 4 : 2.5}>
          <SwiperSlide>
            <p
              onClick={() => setRouter("Unpaid")}
              className={`${
                router == "Unpaid" ? "border-b-2 border-warning font-bold" : ""
              } h-9 lg:mx-20 cursor-pointer`}
            >
              Unpaid
            </p>
          </SwiperSlide>
          <SwiperSlide>
            <p
              onClick={() => setRouter("Shipped")}
              className={`${
                router == "Shipped" ? "border-b-2 border-warning font-bold" : ""
              } h-9 lg:mx-20 cursor-pointer`}
            >
              Shipped
            </p>
          ...
          <SwiperSlide>
            <p
              onClick={() => setRouter("Received")}
              className={`${
                router == "Received"
                  ? "border-b-2 border-warning font-bold"
                  : ""
              } h-9 lg:mx-20 cursor-pointer`}
            >
              Received
        ...
        <>
          {data.length >= 1 ? (
              ...
              {data.map((item: any, index: number) => {
                return (
                  <div key={index} className="bg-white mt-3">
                    {router == "Unpaid" ? (
                      <CardHistory
                        handleDetailHistory={() => handleDetailHistory(index)}
                        titleBtn="Pay"
                        route="Unpaid"
                        title="to pay"
                      ...
                      <CardHistory
                        handleDetailHistory={() => handleDetailHistory(index)}
                        route="Received"
                        orderReceived={() => orderReceived(data[index].id)}
                        titleBtn="Order Received"
                        title="Received"
                        transactionDetails={item.transaction_details}
                        totalPrice={item.total_price}
            ...
            <div className="flex justify-center mt-[30vh]">
              <img
                src={clipboard}
                className="w-2/6 md:w-1/6 lg:w-1/12"
                alt=""
              />
            </div>
          )}
