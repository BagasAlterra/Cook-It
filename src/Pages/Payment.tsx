...
const Payment = () => {
  ...
  const handlePayment = () => {
    setLoading(true);
    axios
      .post("https://cookit.altapro.online/login", {
      ...
      .then((response) => {
        axios
          .post(
            "https://cookit.altapro.online/users/transactions",
            {
      ...
      <div className="px-5 py-5 grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
              {delivery.map((item: any, index: number) => {
                return (
                  <div
                    key={index}
                    onClick={() => handleDelivery(index, item.name, item.price)}
                    className={`${
                      index == thisDelivery
                        ? "border-2 border-primary shadow-none"
                        : ""
                    } h-16 py-2 px-3 grid gap-2 grid-cols-5 items-center cursor-pointer justify-between rounded-md bg-gray-100 shadow-md`}
                  >
                    <img
                      src={
                        index == 0
                    ...
                    <p className="text-xs md:text-[10px] text-primary">
                      <Format>{item.price}</Format>{" "}
                    </p>
                  </div>
                );
        ...
        <div className="bg-gray-100 realtive px-3 pb-8 rounded-md">
          ...
          <div className="">
            <label
              htmlFor="my-modal-6"
              className="btn w-full px-2 py-2 grid grid-cols-5 items-center justify-center text-start rounded-md hover:bg-gray-100 bg-gray-100 shadow-md "
            >
            ...
            <div className="grid grid-cols-2 justify-between my-5 gap-4">
              ...
              <p className="text-end">
                <Format>{totalPrice}</Format>{" "}
              </p>
              <p>Shipping subtotal</p>
              <p className="text-end">
                <Format>{deliveryName ? deliveryName : 0}</Format>{" "}
              </p>
              <p>Order Total</p>
              <p className="text-end">
                <Format>{totalPrice + deliveryName}</Format>
              </p>
            </div>
        ...
        <div className="modal modal-bottom sm:modal-middle">
          <div className="modal-box">
            ...
            <div>
              ...
              <label
                onClick={() => setPaymentOptions("BCA")}
                htmlFor="my-modal-6"
                className="border-b-2 py-4 pl-10 flex items-center gap-4"
              >
                <img className="w-8" src={bca} alt="" />
                Bank BCA
              </label>
              <label
                onClick={() => setPaymentOptions("BNI")}
                htmlFor="my-modal-6"
                className="border-b-2 py-4 pl-10 flex items-center gap-4"
              >
