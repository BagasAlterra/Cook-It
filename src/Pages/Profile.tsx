...
const Profile = () => {
  ...
  const fetchUserPosts = async () => {
    try {
      const response = await axios.get(
        `${endpoint}/recipes?page=0&limit=${limit}&user_id=${userID}`,
        {
          headers: {
            Accept: "application/json",
            Authorization: `Bearer ${cookies.user.token}`,
  ...
  const handleFollow = () => {
    ...
    fetch(`https://cookit.altapro.online/users/follow/${userID}`, headers)
      .then((response) => response.json())
      .then((data) => {
        ...
        <>
              ...
              <div className="grid grid-cols-3 lg:grid-cols-3 w-full gap-2">
                ...
                <div className="grid grid-cols-2 place-content-center gap-5 col-span-2 items-center">
                  ...
                  <div className="absolute top-20 right-6">
                    {userID == cookies.user.id ? (
                      <button
                        ...
                        className=" text-primary place-self-end mb-24 text-2xl rounded-full"
                      >
                        <BsPencilSquare />
                      </button>
                    ) : (
                      <button
                        ...
                        className=" btn-primary place-self-end mb-24 w-fit px-7 py-1 rounded-full"
                      >
                        Follow
                      </button>
                    )}
                  ...
                  {cookies.user.id == userID ? (
                    <>
                      <p
                        onClick={() => goToFollow("Followers")}
                        className="font-regular sm:text-sm md:text-lg lg:text-lg md:font-regular cursor-pointer"
                      >
                        {userData.followers} Followers
                      </p>
                      <p
                        onClick={() => goToFollow("Following")}
                        className="font-regular sm:text-sm md:text-lg lg:text-lg md:font-regular cursor-pointer"
                      >
          ...
          {userPosts.length > 0 ? (
                    ...
                    {post.replied_recipe !== undefined ? (
                      <>
                        <CardQuote
                          ...
                          recipePicture={
                            post.replied_recipe.images[0].url_image
                          }
                          verifiedUser={
                            post.replied_recipe.user_role === "VerifiedUser"
                          }
                          verifiedRecipe={
                            post.replied_recipe.status === "OpenForSale"
                          }
                        />
                      </>
                    ) : (
            ...
            <div className="text-center">
              <p className="pt-32 text-2xl font-semibold">
                Share your recipe now
              </p>
              <p>
                <span
                  onClick={() => navigate("/newcooking")}
          ...
          {userPosts.length < 3 ? (
            ...
            <button
              className={`w-full ${
                loadnew ? "animate-pulse bg-neutral-100" : ""
              } border-x-2 text-neutral-400 py-2 text-light hover:bg-neutral-100`}
