...
const Recipe = () => {
  ...
  const fetchRecipeDetails = async () => {
    try {
      const response = await axios.get(
        `${endpoint}/recipes/${recipeID}/detail`,
        {
          headers: {
  ...
  const addToCart = (id: number) => {
    axios
      .post(
        `https://cookit.altapro.online/users/carts`,
        {
          ingredient_id: id,
          quantity: 1,
        },
  ...
  const fetchComments = async () => {
    try {
      const response = await axios.get(
        `${endpoint}/recipes/${recipeID}/comments?page=0&limit=10`,
        {
          headers: {
            Accept: "application/json",
            Authorization: `Bearer ${cookies.user.token}`,
  ...
  const handleDeletePost = () => {
    Swal.fire({
      ...
      if (willDelete.isConfirmed) {
        axios
          .delete(`https://cookit.altapro.online/recipes/${recipeID}`, {
            headers: {
              Authorization: `Bearer ${cookies.user.token}`,
              Accept: "application/json",
            },
              ...
              <div className="flex flex-col justify-center items-center text-center">
                ...
                <div className="grid grid-cols-3 gap-10 text-secondary justify-items-center mt-1">
                  ...
                  <div className="flex justify-self-end dropdown dropdown-end">
                    ...
                    >
                      <li
                        onClick={() => navigate(`/recipes/${recipeID}/reply`)}
                      >
                        <p>
                          <MdOutlineReply />
                          Reply
                      ...
                      <li
                        ...
                        className={`${
                          recipe.type === "Cooked" ? "hidden" : "block"
                        }`}
                      >
                        <p>
                          <ImLoop2 />
                      ...
                      <li
                        className={`${
                          recipe.user_id == cookies.user.id ? "block" : "hidden"
                        }`}
                      >
                        <Link to={`edit`}>
                          <MdModeEdit />
                ...
                <div className="w-full flex flex-col gap-2">
                  ...
                  <div className="flex items-center font-bold">
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        handleChangeServing(Number(event.target.value))
                      }
                      min={1}
                    />
                    <button
                  ...
                  <table className="table table-auto table-zebra -z-10 w-full">
                    <tbody>
                      {recipe.ingredients[0].ingredient_details.map(
                  ...
                  {recipe.status !== "None" ? (
                    <div className="flex flex-col gap-5 border-2 border-primary mt-1 px-2 pb-2">
                      <p className="self-center text-primary font-semibold bg-white -mt-3 px-2">
                        Buy Ingredients
                      </p>
                      <p>
                        {
                          "This is a verified recipe, you can directly purchase the ingredients for "
                        }
                        <span className="text-primary">
                          <Format>{recipe.ingredients[0].price}</Format> / batch
                        </span>
                      </p>
                      <div className="grid grid-cols-2 items-center font-bold">
                        ...
                        <button
                          onClick={() => addToCart(recipe.ingredients[0].id)}
                          className="btn btn-secondary w-40 justify-self-end"
                        >
                          Add to Cart
                        </button>
                      </div>
              ...
              {recipe.replied_recipe !== undefined ? (
                <>
                  <CardQuote
                    ...
                    verifiedUser={
                      recipe.replied_recipe.user_role === "VerifiedUser"
                    }
                    verifiedRecipe={
                      recipe.replied_recipe.status === "OpenForSale"
                    }
                  />
                </>
              ) : (
