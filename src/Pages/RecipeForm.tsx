...
const RecipeForm = () => {
  ...
  const fetchRecipeDetails = async () => {
    try {
      ...
      if (
        (cookies.user.id !== response.data.data.user_id &&
          editType === "edit") ||
      ...
      const steps =
        response.data.data.steps !== undefined
          ? response.data.data.steps.map((item: any) => item.name)
          : [""];
      setSteps(steps);
      const ingredients =
        response.data.data.ingredients !== undefined
          ? response.data.data.ingredients[0].ingredient_details.map(
              ({ id, ...rest }: any) => rest
            )
          : ["", 0, ""];
  ...
  const [newRecipeDetails, setNewRecipeDetails] =
    useState<NewRecipeVariables>(initialNewRecipe);
    ...
    if (newRecipeDetails.images) {
      try {
        ...
        const response = await axios.post(
          `${endpoint}/recipes/${recipeID}/images`,
          formData,
          {
            headers: {
              Accept: "application/json",
  ...
  const postRecipe = async () => {
    ...
    try {
      ...
      const data = {
        ...
        ingredients: ingredients,
        recipe_id:
          editType === "recook" && recipeID !== undefined
            ? parseInt(recipeID)
            : null,
        type: editType === "recook" ? "Mixed" : "Original",
      };

      const response = await axios.post(`${endpoint}/recipes`, data, {
        headers: {
          Accept: "application/json",
          // "Content-Type": 'multipart/form-data',
          Authorization: `Bearer ${cookies.user.token}`,
  ...
  return (
    <Layout>
      ...
      {recipeID !== undefined ? (
        editType === "edit" ? (
          <NavBack title="Edit Recipe" />
        ) : (
          <NavBack title="Recook Recipe" />
        )
      ) : (
        <NavBack title="New Recipe" />
      )}
      ...
      {loading ? (
        ...
        <>
          <form
            className="w-full flex flex-col my-2 gap-2 px-4"
          ...
          >
            ...
            <div className="flex flex-col gap-2">
              ...
              {ingredients.map((ingredient, index) => (
                <div className="grid grid-cols-12 gap-2 " key={index}>
                  ...
                  <button
                    type="button"
                    className={`btn btn-sm btn-circle btn-ghost col-span-1 ${
                      index == 0 ? "hidden" : ""
                    }`}
            ...
            <div
              className={`items-center gap-1 mt-2 ${
                userRole === "VerifiedUser" ? "flex" : "hidden"
              }`}
            >
              ...
              <input
                ...
                checked={showPrice}
                type="checkbox"
                className="checkbox checkbox-primary rounded-full checkbox-sm"
              />
            </div>
            ...
            <button className="btn btn-primary w-1/2 self-end mt-2">
              Submit
            </button>
          </form>
        </>
