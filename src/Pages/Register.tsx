...
const Register = () => {
  ...
  const handleRegister = (e: any) => {
    e.preventDefault();
    axios
      .post("https://cookit.altapro.online/register", {
        username: username,
        email: email,
        password: password,
      })
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 row-end-3 h-screen relative overflow-hidden justify-center items-center">
      <Alert
        type={alert}
        message="Please input your email, username and password.."
      />
      ...
      <div className="w-full h-full px-5 lg:mb-16 md:h-[60vh] flex flex-col items-center">
        ...
        <form className="sm:w-72 lg:px-10 md:w-2/3 lg:w-72 px-5 pb-8 text-center flex flex-col items-center  ">
          ...
          <div className="text-start pt-3 text-xs">
            ...
            <div className="px-2">
              <p className={`flex ${length ? "text-green-600" : ""}`}>
                {length ? <BiCheck /> : ""} At least 8 characters with no space
              </p>
              <p className={`flex ${upperCase ? "text-green-600" : ""}`}>
                {upperCase ? <BiCheck /> : ""} At least 1 upper case letter
              </p>
          ...
          <button
            ...
            className="w-full md:w-96 lg:w-full h-8 my-5 rounded-lg text-white font-semibold bg-orange-500"
          >
            Sign up
          </button>
          <p className="text-sm">
      ...
      {responsive < 767 ? (
        <img
          className=" -z-10 mt-[95vh] ml-[20vw] absolute"
          src={graph}
          alt=""
        />
      ) : (
